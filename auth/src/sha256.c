...
void calculate_sha256(FILE *file) {
    ...
    __uint32_t K[] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
...
int fillMessageBlock(FILE *file, union messageBlock *msgBlock,
    ...
    if (*state == PAD0 || *state == PAD1) {
        ...
        for (i = 0; i < 56; i++) { msgBlock->e[i] = 0x00; }
    ...
    *numBits = *numBits + (numBytes * 8);
    ...
    if (numBytes < 56) {
        ...
        // Store the length of the file in bits as a (Should be big endian)
        // unsigned 64 bit int
        msgBlock->s[7] = byteSwap64(*numBits);
    ...
    // Otherwise if we're at the end of the file, need to create a new message
    // block full of padding
    else if (feof(file)) {
...
// This function is used to read the contents of the file and return them as an
// array of chars
